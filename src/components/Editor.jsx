      ...
      <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
        ...
        >
          ...
          <div className="tool-dropdown">
            ...
            {exportOpen && (
              <>
                <div className="dropdown-overlay" onMouseDown={() => setExportOpen(false)} />
                <div className="dropdown-menu">
                  <button className="dropdown-item" onClick={() => { exportAsPdf(note.title, titleRef.current?.innerHTML || '', contentRef.current?.innerHTML || '', note.bgColor, note.titleAlign || 'center'); actions.showToast('PDF exported'); setExportOpen(false); }}>
                    <i className="fa-solid fa-file-pdf"></i> Export as PDF
                  </button>
                  <button className="dropdown-item" onClick={() => { const allHtml = (titleRef.current?.innerHTML || '') + (contentRef.current?.innerHTML || ''); exportAsText(note.title, allHtml); actions.showToast('Text exported'); setExportOpen(false); }}>
                    <i className="fa-solid fa-file-lines"></i> Export as Text
                  </button>
                  <button className="dropdown-item" onClick={() => { window.print(); setExportOpen(false); }}>
